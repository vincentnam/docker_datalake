...
class ModelEditForm extends React.Component {
    ...
    submitModels(event) {
        ...
        if (nbErrors === 0) {
            const edit = modelEdit(this.props.modelEdit._id, this.state.label, types, this.state.metadonnees, this.state.status, this.props.nameContainer.nameContainer, localStorage.getItem('token'))
            edit.then((response) => {
